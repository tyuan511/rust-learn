...
impl Png {
    ...
    fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk, String> {
        ...
        Err(String::from("No chunk found, can't remove"))
    }
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self, Self::Error> {
        let chunks = generate_chunks(bytes, 8);
        if chunks.is_err() {
...
fn generate_chunks(bytes: &[u8], iter: usize) -> Result<Vec<Chunk>, String> {
    let chunks = match group_bytes_into_chunks(bytes, iter) {
        Ok(chunks) => chunks,
    ...
    let result = chunks
        .par_iter()
        .map(|chunk| Chunk::try_from(*chunk).unwrap())
        .collect();
    Ok(result)
}

fn group_bytes_into_chunks(bytes: &[u8], mut iter: usize) -> Result<Vec<&[u8]>, String> {
    let mut result = vec![];
    while iter < bytes.len() {
        let len = u32::from_be_bytes(chunk::array_from_slice(&bytes[iter..iter + 4])) as usize;
        if iter + 12 + len > bytes.len() {
            return Err(String::from("INvalid chunk"));
        }
        result.push(&bytes[iter..iter + 12 + len]);
        iter += 12 + len;
    }
    Ok(result)
}
